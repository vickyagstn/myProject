...
export default function ArisanUang() {
  ...
  const fetchData = async () => {
    ...
    const q = query(collection(db, 'arisan_uang'), orderBy('putaran'))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await updateDoc(doc(db, 'arisan_uang', editId), {
        ...
        showNotif('✅ Data arisan berhasil disimpan!')
      }
  ...
  const handleHapus = async (id) => {
    ...
    await deleteDoc(doc(db, 'arisan_uang', id))
    fetchData()
    showNotif('🗑️ Data berhasil dihapus!')
  }

  return (
    <Layout title="Arisan Uang">
      {notif && (
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Kelola penerima arisan uang</p>
        </div>
        <button onClick={() => { setShowForm(!showForm); setEditId(null); setForm({ penerima: '', jumlah: '', tanggal: '', putaran: '' }) }}
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-gradient-to-br from-yellow-500 to-amber-400 rounded-2xl p-4 text-white">
      ...
      {showForm && (
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 mb-6">
          <h2 className="font-bold text-gray-800 mb-4">
            {editId ? '✏️ Edit Data Arisan' : '➕ Tambah Penerima Arisan'}
          </h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Penerima</label>
              <select className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-400 bg-gray-50"
            ...
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Jumlah Arisan (Rp)</label>
              <input type="number"
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-400 bg-gray-50"
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100">
          <h3 className="font-bold text-gray-800">📋 Riwayat Penerima</h3>
        </div>
